...
def infer_normality_class(text_iter, norm_model_path, batch_size=128, n_procs=1):
    nlp_norms = spacy.load(norm_model_path)
    norm_classes = [
        normality_class(doc)
        for doc in tqdm(
            nlp_norms.pipe(text_iter, batch_size=batch_size, n_process=n_procs)
        )
    ]
    return norm_classes


def infer_comparitive_class(text_iter, comp_model_path, batch_size=128, n_procs=1):
    nlp_comps = spacy.load(comp_model_path)
    comp_classes = [
        is_comparitive(doc)
        for doc in tqdm(
            nlp_comps.pipe(text_iter, batch_size=batch_size, n_process=n_procs)
        )
    ]
    return comp_classes


def infer_age_at_study(dataframe):
    return (dataframe["End Exam Date"] - dataframe["Patient DOB"]).dt.days / 365


def infer_report_length(text_iter, token_model_path, batch_size=128, n_procs=1):
    nlp = spacy.load(
        token_model_path,
        exclude=["parser", "tagger", "ner", "lemmatizer", "relex", "negex"],
    )
    lengths = [
        doc_word_length(doc)
        for doc in tqdm(nlp.pipe(text_iter, batch_size=batch_size, n_process=n_procs))
    ]
    return lengths


def infer_sections(
    text_iter, token_model_path, section_cls_path, batch_size=128, n_procs=1
):
    sectioner = SpacySectioner(token_model_path, section_cls_path)
    sectioned_reports = sectioner(text_iter, batch_size=batch_size, n_procs=n_procs)
    return sectioned_reports


def infer_pathological_domains(
    text_iter, token_model_path, domain_model_path, batch_size=128, n_procs=1
):
    nlp = spacy.load(
        token_model_path, exclude=["parser", "tagger", "lemmatizer", "relex"]
    )
    domain_model = DomainDetector(nlp)
    domain_model.from_disk(domain_model_path)
    domains = [
        domain_model(doc)._.domains
        for doc in tqdm(nlp.pipe(text_iter, batch_size=batch_size, n_process=n_procs))
    ]
    return domains
