#!/usr/bin/env python
"""Preprocess negation data.

Create some negation data with the spacy ner pipeline and save as training data for huggingface pipeline
"""
...
def convert_ents(doc):
    new_ents = [
        Span(doc, e.start, e.end, label="negated")
        if e._.negex
        else Span(doc, e.start, e.end, label="asserted")
        for e in doc.ents
    ]
    doc.ents = new_ents
    return doc
...
types = [
    ...
    "descriptor-flow",
    "descriptor-interval-change",
    "descriptor-mass-effect",
    "descriptor-morphology",
    "descriptor-necrosis",
    "descriptor-signal-change",
]
...
def main():
    ...
    nlp = spacy.load(args.model, exclude=["negation"])
    nlp.add_pipe("negex", config={"ent_types": types})
