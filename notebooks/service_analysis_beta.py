...
from neuroNLP.dashboard.utils import df_to_json, generate_service_report_pdf
...
def transform_data(data, start_date, end_date):
    """perform the necessary transformation on the input data"""
    start = pd.to_datetime(start_date, dayfirst=True, infer_datetime_format=True)
    end = pd.to_datetime(end_date, dayfirst=True, infer_datetime_format=True)
...
def per_reporter_analysis(df, output_dir):
    ...
    for r in all_reporters:
        ...
        ax1_.grid(False)
        pathology_columns = [i for i in merged_reporters.columns if "has-" in i]
        renamed = {i: i.replace("has-", "") for i in pathology_columns}
        pathology_counts = (
        ...
        ax2.grid(True)
        fig.tight_layout()
        output_results(name, df_to_json(table), fig, output_dir, 20, 14, start, end)
        plt.close()


# %%
def calculate_contrast_proportion(df):
    pathology_columns = [i for i in df.columns if "has-" in i] + ["date"]
...
def create_rolling_average(df):
    pathology_columns = [i for i in df.columns if "has-" in i]
    for c in pathology_columns:
...
def contrast_usage_report(df, output_dir):
    ...
    contrast_proportion = create_rolling_average(contrast_proportion).rename(
        columns=renamed
    )
    has_contrast = contrast_usage[contrast_usage["uses_contrast"]].drop(
        columns=["uses_contrast"]
    )
    has_contrast = create_rolling_average(has_contrast).rename(columns=renamed)
...
# %%
INPUT = Path("DATAPATH/Desktop/neuroData/processed_reports/processed_reports.csv")
OUTDIR = Path("DATAPATH/Dropbox/report_test")
...
transformed_data = transform_data(data, start_date, end_date)

# %%
transformed_data["has-cerebrovascular"] = transformed_data[
...
# %%200
aggregate_reporters_report(transformed_data, OUTDIR)
# %%
per_reporter_analysis(transformed_data, OUTDIR)
# %%
contrast_usage_report(transformed_data, OUTDIR)
